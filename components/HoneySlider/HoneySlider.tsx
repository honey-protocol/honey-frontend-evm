...
export const HoneySlider: FC<HoneySliderProps> = ({
	...
	const currentSliderValue =
		(currentValue / (maxValue * maxAvailablePosition - minAvailableValue)) * 100;

	const isRisky = (currentValue + minAvailableValue) / maxValue > maxSafePosition;

	const isDanger = (currentValue + minAvailableValue) / maxValue > dangerPosition;
		...
		<div className={styles.rangeContainer}>
			...
			>
				...
				<Slider
					className={c(
						styles.slider,
						isRisky ? styles.enabledWarningBackgroundSlider : styles.enabledBackgroundSlider
					)}
			...
			<div
				className={styles.sliderWrapper}
				style={{
					display: availablePosition <= 0 ? 'none' : 'flex',
					width: availablePosition ? `${availablePosition * 100}%` : '100%'
				}}
			>
				{!isReadonly && (
