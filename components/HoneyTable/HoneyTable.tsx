...
const HoneyTable: FC<HoneyTableProps> = (props) => {
	const { className, rowClassName, expandedRowClassName, expandable, hasRowsShadow, ...rest } =
		props;

	const isExpandedRow = (key: Key): boolean => {
		if (!expandable || !expandable.expandedRowKeys || !expandable.expandedRowKeys.length) {
			return false;
		}
	...
	const getRowExternalClassName = (record: any, index: number, indent: any): string => {
		if (!rowClassName) {
			return '';
		}
	...
	const _rowClassName = (record: any, index: number, indent: any) => {
		const key = props.rowKey && typeof props.rowKey === 'string' ? props.rowKey : 'key';
	...
	const getExpandedRowExternalClassName = (record: any, index: number, indent: any): string => {
	...
	const _expandedRowClassName = (record: any, index: number, indent: any) => {
		const externalExpandedRowClassName = getExpandedRowExternalClassName(record, index, indent);
