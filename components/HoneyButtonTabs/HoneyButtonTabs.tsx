...
export const HoneyButtonTabs: FC<HoneyButtonTabsProps> = ({
	...
	<div
		className={c(style.periodSelector, {
			[style.fullWidthSelector]: isFullWidth
		})}
	>
		<div
			className={c(style.periodSelectorContent, {
				[style.periodSelectorContentResponsibility]: !isFullWidth
			})}
		>
			{items.map((item, index) => {
				const itemStatus = item.slug == activeItemSlug ? 'active' : 'passive';
				const isVisibleDivider = items.length - 1 !== index && itemStatus !== 'active';
				return (
					<div
						key={item.slug}
					...
					>
						{item.nameMobile ? (
							<>
								<div className={style.hideNameMobile}>{item.name}</div>
								<div className={style.showNameMobile}>{item.nameMobile}</div>
							</>
						) : (
							<>{item.name}</>
						)}
