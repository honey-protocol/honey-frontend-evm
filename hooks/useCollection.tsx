      ...
      return {
        ...
        queryFn: async () => {
          if (walletPublicKey != "" && ERC721ContractAddress != "") {
            try {
              const metaData = await getMetaDataFromNFTId(
                ERC721ContractAddress,
                coupon.NFTId
              );
              const debt = Number(coupon.borrowAmount);
              const available = maxBorrowAmountNumber
                ? maxBorrowAmountNumber - debt
                : 0;
              const result: MarketTablePosition = {
                ...
                available,
                healthLvl:
                  ((nftPrice - debt / LIQUIDATION_THRESHOLD) / nftPrice) * 100
              };
              return result;
            } catch (e) {
